...
from blenheim.config import Config, default_config
...
from blenheim.schema.authentication.authentication import (
    Authentication,
    authenticate,
    TOKENS,
    USERS,
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        ...
        with patch.object(Authentication, "expire_tokens", lambda: None):
            info = SimpleNamespace(
                context={
                    "request": SimpleNamespace(headers={"Authorization": token}),
                    "current_user": user,
                }
            )
...
def test_settings_mutations(tmp_path):
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        ...
        with patch.object(Authentication, "expire_tokens", lambda: None):
            ...
            res = CreateDomain().mutate(
                info, id="ex.com", subdomains=[{"subdomain": "a"}]
            )
...
async def test_settings_resolvers(tmp_path):
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        ...
        assert (
            await settings.resolve_default_subdomains(info)
            == cfg["settings"]["default_subdomains"]
        )
...
@patch(
    ...
    return_value=[("ex.com", "data")],
)
@patch(
    "blenheim.library.dns.named_conf_local.NamedConfLocal.generate_named_conf_local",
    return_value="conf",
)
...
@pytest.mark.asyncio
async def test_dns_generate(
    m_open, mock_from_env, mock_root, mock_docker, mock_conf, mock_zone, tmp_path
):
    cfg_file = tmp_path / "config.json"
    with patch("blenheim.config.FILENAME", cfg_file):
        cfg = Config()
        ...
        mock_from_env.return_value = SimpleNamespace(
            containers=SimpleNamespace(
                list=lambda filters: [SimpleNamespace(restart=lambda: None)]
            )
        )
        dns = dnsmod.Dns()
        patcher = patch.object(Authentication, "expire_tokens", lambda: None)
...
async def test_dns_generate_errors(tmp_path):
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        ...
        info = SimpleNamespace(
            context={
                "request": SimpleNamespace(headers={"Authorization": "t"}),
                "current_user": cfg[USERS]["admin"],
            }
        )
        with patch.object(Authentication, "expire_tokens", new=lambda: None):
            with patch(
                "blenheim.schema.dns.dns.check_docker_container",
                return_value=Result("bad"),
            ):
            ...
            with patch("blenheim.schema.dns.dns.check_root", return_value=None), patch(
                ...
                "blenheim.schema.dns.dns.from_env",
...
@pytest.mark.asyncio
async def test_dns_zonefile_write_error(
    mock_conf, mock_zone, mock_root, mock_docker, tmp_path
):
    cfg_file = tmp_path / "config.json"
    with patch("blenheim.config.FILENAME", cfg_file):
        cfg = Config()
        ...
        with patch.object(Authentication, "expire_tokens", lambda: None), patch(
...
@pytest.mark.asyncio
async def test_dns_namedconf_write_error(
    mock_conf, mock_zone, mock_root, mock_docker, tmp_path
):
    cfg_file = tmp_path / "config.json"
    with patch("blenheim.config.FILENAME", cfg_file):
