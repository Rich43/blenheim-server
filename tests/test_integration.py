...
def test_graphql_login_and_settings(tmp_path):
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        with TestClient(app) as client:
            login_query = 'mutation { authentication { login(details:{name:"admin", password:"Password1"}) } }'
...
def test_graphql_dns_generate(tmp_path):
    ...
    with patch("blenheim.config.FILENAME", cfg_file):
        with TestClient(app) as client:
            ...
            with patch("blenheim.schema.dns.dns.from_env") as mock_from_env, patch(
                "blenheim.schema.dns.dns.check_root", return_value=None
            ), patch(
